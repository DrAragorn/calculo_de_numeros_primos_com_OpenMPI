...
/* Envia pedaços com TAMANHO números para cada processo */
    if (meu_ranque == 0) {
        ...
        for (dest = 1; dest < num_procs; dest++) {
            if (inicio < n) {
                MPI_Ssend(&inicio, 1, MPI_INT, dest, 1, MPI_COMM_WORLD); // trabalho
                inicio += TAMANHO;
                enviados++;
            } else {
                int fim = 0;
                MPI_Ssend(&fim, 1, MPI_INT, dest, 99, MPI_COMM_WORLD); // aviso de término
            }
        }
        ...
        while (enviados > 0) {
            ...
            if (inicio < n) {
                MPI_Ssend(&inicio, 1, MPI_INT, escravo, 1, MPI_COMM_WORLD);
                inicio += TAMANHO;
                ...
                int fim = 0;
                MPI_Ssend(&fim, 1, MPI_INT, escravo, 99, MPI_COMM_WORLD);
                enviados--;
